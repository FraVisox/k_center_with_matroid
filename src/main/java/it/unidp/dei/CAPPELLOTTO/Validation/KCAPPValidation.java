...
public class KCAPPValidation implements Algorithm {

    public KCAPPValidation(int[] _ki, double _beta, double _minDist, double _maxDist) {
        ...
        guesses = new KGuessValidation[number_of_guesses];
        ...
        for (int i = 0; i<number_of_guesses; i++) {
            guesses[i] = new KGuessValidation(gamma, _ki);
            gamma *= (1+_beta);
        }
    }
    ...
    public int getSize() {
        int size = 0;
        for (KGuessValidation g : guesses) {
            size += g.getSize();
        }
        return size;
    }
    ...
    //Array of guesses
    private final KGuessValidation[] guesses;
}
